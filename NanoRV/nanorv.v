...
module NrvALU(
  ...
  output reg [31:0] out,   // (latched) ALU result
...
`ifdef NRV_TWOSTAGE_SHIFTER
	 if (shamt > 4) begin
	    ...
	    case(op)
              ...
              3'b101: shifter <= $signed({opqual ? shifter[31] : 1'b0, shifter}) >>> 4 ; // SRL/SRA
	    endcase 
	 end else  
`endif
	 ...
	 if (shamt != 0) begin
	    ...
	    case(op)
              ...
              3'b101: shifter <= $signed({opqual ? shifter[31] : 1'b0, shifter}) >>> 1 ; // SRL/SRA
	    endcase 
	 end
...
module NrvProcessor(
   ...
   initial instrAddress = 32'h00000000;
